 ...
 * or query hints on them. This version copies most of the functionality of the version declared in package
 * {@link org.springframework.data.jpa.repository.support}, but with additional support for dynamic entity graph
 * control. <br/>
 * <br/>
 * This implementation is a bit hacky, and it's not clear that the metadata stored in the
 * TransactionSynchronizationManager is intended to be part of the public JPA API contract. Use carefully, and look
 * first for changes to CrudMethodMetadataPostProcessor if future JPA updates break this implementation.
 *
 ...
 * @author Ben Giddens
 */
class ProjectionEntityGraphAwareCrudMethodMetadataPostProcessor
	...
	public ProjectionEntityGraphAwareCrudMethodMetadataPostProcessor(
	...
	CrudMethodMetadata getCrudMethodMetadata() {
		...
		factory.setTargetSource(new ProjectionEntityGraphAwareCrudMethodMetadataPostProcessor.ThreadBoundTargetSource());
		...
		public @Nullable Object invoke(MethodInvocation invocation) throws Throwable {
			...
			try {
				...
				if (methodMetadata == null) {
					...
					final var projectionType = projectionContextProvider
							.getCurrentProjection(repositoryInformation.getDomainType());
					if (projectionType == null) {
						return invocation.proceed();
					}
					final var graph = projectionEntityGraphRegistry
							.getEntityGraphForProjection(repositoryInformation.getDomainType(), projectionType);

					methodMetadata = new ProjectionEntityGraphAwareCrudMethodMetadataPostProcessor.DefaultCrudMethodMetadata(
							method, graph);
				...
				if (TransactionSynchronizationManager.hasResource(method)) {
					// We're replacing a previous post-processors bindings, which will expect them to be present when this
					// finishes.
	...
	private static class DefaultCrudMethodMetadata implements CrudMethodMetadata {
		 ...
		 */
		DefaultCrudMethodMetadata(Method method, @Nullable EntityGraph entityGraph) {
			...
			this.entityGraph = (entityGraph != null) ? entityGraph : findEntityGraph(method);
			this.method = method;
		}
	...
	private static class ThreadBoundTargetSource implements TargetSource {
		...
		public @Nullable Object getTarget() {
			...
			MethodInvocation invocation = ProjectionEntityGraphAwareCrudMethodMetadataPostProcessor.CrudMethodMetadataPopulatingMethodInterceptor
					.currentInvocation();
